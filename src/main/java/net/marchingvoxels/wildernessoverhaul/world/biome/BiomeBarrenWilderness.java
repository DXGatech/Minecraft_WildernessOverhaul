...
package net.marchingvoxels.wildernessoverhaul.world.biome;
...
import net.marchingvoxels.wildernessoverhaul.entity.EntityBrigand;
import net.marchingvoxels.wildernessoverhaul.ElementsWildernessOverhaul;

import java.util.Random;

@ElementsWildernessOverhaul.ModElement.Tag
public class BiomeBarrenWilderness extends ElementsWildernessOverhaul.ModElement {
	@GameRegistry.ObjectHolder("wildernessoverhaul:barrenwilderness")
	public static final BiomeGenCustom biome = null;
	public BiomeBarrenWilderness(ElementsWildernessOverhaul instance) {
		super(instance, 10);
	}
	...
	public void init(FMLInitializationEvent event) {
		BiomeDictionary.addTypes(biome, BiomeDictionary.Type.COLD, BiomeDictionary.Type.SPARSE);
		BiomeManager.addSpawnBiome(biome);
		BiomeManager.addBiome(BiomeManager.BiomeType.COOL, new BiomeManager.BiomeEntry(biome, 1000));
	}
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Barren Wilderness").setRainfall(0.1F).setBaseHeight(0.2F).setWaterColor(-16724788)
					.setHeightVariation(0.32F).setTemperature(0.2F));
			setRegistryName("barrenwilderness");
			...
			this.spawnableCreatureList.add(new SpawnListEntry(EntityBrigand.EntityCustom.class, 40, 1, 5));
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.GRASS.getDefaultState().getBlock() || ground == Blocks.DIRT.getStateFromMeta(2).getBlock())
							&& (ground2 == Blocks.GRASS.getDefaultState().getBlock() || ground2 == Blocks.DIRT.getStateFromMeta(2).getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), Blocks.DIRT.getStateFromMeta(2), 2);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == Blocks.LEAVES2.getStateFromMeta(1).getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, Blocks.LEAVES2.getStateFromMeta(1));
										}
									}
								}
							}
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.LEAVES2.getStateFromMeta(1).getBlock() || blockType == Blocks.GRASS.getDefaultState().getBlock()
					|| blockType == Blocks.DIRT.getStateFromMeta(2).getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != Blocks.DIRT.getStateFromMeta(2).getBlock())
				this.setBlockAndNotifyAdequately(world, pos, Blocks.DIRT.getStateFromMeta(2));
		}
